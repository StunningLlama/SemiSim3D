...
public class SegmentedPath extends Path {
	...
	public double getZ(double t) {
		int i = 0;
		while (t > arclength[i]) i++;
		if (i >= arclength.length) return vz.get(arclength.length-1);
		if (i == 0) return vz.get(0);
		
		double f = (t - arclength[i-1])/(arclength[i] - arclength[i-1]);
		return vz.get(i)*f + vz.get(i-1)*(1-f);
	}

	@Override
	public void draw(Renderer r) {
		assert(vx.size() == vy.size());
		assert(vx.size() == vz.size());
		if (vx.size() > 0) {
			r.setalphaBG(0.5);
			r.setalphaFG(1.0);
			r.setColorFloat(1.0f, 1.0f, 1.0f);
			for (int i = 0; i < vx.size() - 1; i++) {
				r.drawPixelLine((int)vx.get(i), (int)vy.get(i), (int)vz.get(i), (int)vx.get(i+1), (int)vy.get(i+1), (int)vz.get(i+1));
			}

			r.setalphaBG(0);
			r.setalphaFG(1.0);
			r.setColorFloat(0.7f, 0.7f, 0.7f);
			r.drawPixelRectangle((int)vx.get(0)-1, (int)vy.get(0)-1, (int)vz.get(0)-1, 3, 3, 3);
			r.setColorFloat(1.0f, 1.0f, 1.0f);
			r.drawPixelRectangle((int)vx.get(vx.size()-1)-1, (int)vy.get(vy.size()-1)-1, (int)vz.get(vz.size()-1)-1, 3, 3, 3);
		}
	}

	public void addNewJoint(int x, int y, int z) {
		if (vx.size() > 0) {
			if (vx.get(vx.size()-1) == x && vy.get(vy.size()-1) == y && vz.get(vz.size()-1) == z) {
				completed = true;
				return;
			}
		}
	...
	public void computeArc() {
		...
		for (int i = 0; i < vx.size()-1; i++) {
			arclength[i] = l_total;
			l_total += Utils.length(vx.get(i+1)-vx.get(i), vy.get(i+1)-vy.get(i), vz.get(i+1)-vz.get(i));
		}
